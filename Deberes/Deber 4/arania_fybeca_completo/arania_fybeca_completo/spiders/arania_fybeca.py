...
class AraniaCrawlFybeca(CrawlSpider):
    ...
    def parse_page(self, response):
        ...
        for (nombre, precio,precio_afiliado) in zip(nombres, precios_final,precios_miembro_final): 
            with open('fybeca_completo.txt', 'a+', encoding='utf-8') as archivo:
                archivo.write(nombre + '\t'+'No Afiliados: $'+ precio+ '\t'+'Afiliado: $'+precio_afiliado+ '\t' +'Ahorro: $'+str(round((float(precio) - float(precio_afiliado)),2))+'\n')